# ##direction check
execute unless data storage mc:data current_action.action[{type:slider}].direction.right run return fail
...
# ##size limit
scoreboard players operation #mc.gui.temp.slide_size mc.data = #mc.gui.slide.max_x mc.data
