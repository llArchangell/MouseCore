# ##direction check
execute unless data storage mc:data current_action.action[{type:slider}].direction.down run return fail
...
# ##size limit
scoreboard players operation #mc.gui.temp.slide_size mc.data = #mc.gui.slide.min_y mc.data
